...
import { Target, Trophy, Clock, Search, BookOpen, ExternalLink, CheckCircle, Bookmark, MessageCircle, MessageSquare, X, Send } from 'lucide-react';
import { motion } from 'framer-motion';
import { useNavigate } from 'react-router-dom';

const GFGSheet = ({ topics, searchTerm, searchMode, onToggleSolved, onToggleBookmark, onSaveComment }) => {
    ...
    if (isSearching) {
        ...
        matchedQuestions = topics.flatMap(t => {
    ...
    return (
        <div className="container" style={{ paddingTop: '1.5rem' }}>
            <header style={{ marginBottom: '3.5rem', display: 'flex', alignItems: 'center', gap: '3rem', flexWrap: 'wrap' }}>
                <div style={{ flex: 1, minWidth: '350px' }}>
                    <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.75rem', background: '#fff7ed', color: '#ea580c', padding: '0.5rem 1.25rem', borderRadius: '2rem', fontWeight: 800, fontSize: '0.8rem', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '1.5rem', border: '1px solid #ffedd5' }}>
                        <BookOpen size={16} />
                        Love Babbar 450 Sheet
                    </div>
                    <h1 className="section-title">Cracker Series</h1>
                    <p style={{ fontSize: '1.1rem', color: 'var(--text-muted)', lineHeight: '1.8' }}>
                        The legendary 450 DSA questions curated by Love Babbar.
                        Master these to build a solid foundation and crack your dream company.
                    </p>
                </div>
                ...
                {isSearching ? (
                    matchedQuestions.length === 0 ? (
                        ...
                        matchedQuestions.map((q, i) => (
                            ...
                            >
                                <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: '1rem' }} onClick={() => navigate(`/gfg/topic/${q.topicId}`)}>
                    ...
                    >
                        {topics.map((topic) => (
                            <TopicCard key={topic.id} topic={topic} customPath="/gfg/topic" />
                        ))}
                    </motion.div>
                )}
            </div>
